...
export default function Attendance() {
  ...
  //  const filterData = userData.filter((item) => item.first_name.toLowerCase().includes(searchData.toLowerCase() || item.last_name.toLowerCase().includes(searchData.toLowerCase()))
                    ...
                    <Table striped bordered hover className="table m-0">
                      <thead>
                        <tr>
                          ...
                          <th width="13%">No. of Joined</th>
                          <th width="13%">Total attend</th>
                          <th width="10%">Attend Lunch </th>
                          <th width="10%">Received Gift</th>
                      ...
                      <tbody>
                        {isLoading === true ? (
                          <tr>
                            <td colSpan="9" className="text-center p-0">
                              <Dna
                                visible={true}
                                height="80"
                                width="80"
                          ...
                          currentItems?.map((val, index) => {
                            ...
                            return (
                              <tr>
                                ...
                                <td>{val.eventJoinCount}</td>
                                <td>{val.attendanceCount}</td>
                                <td>{val.lunchAttendaceCount}</td>
                                <td>{val.giftAttendaceCount}</td>
                                <td>
                                  {" "}
                                  <Link
                                    href={`./attendance/${val.event_id}`}
                                    onClick={() =>
                                      handleEventName(val.eventname,val.eventJoinCount,val.attendanceCount)
                                    }
                                    style={{ color: "#012970" }}
                                  >
