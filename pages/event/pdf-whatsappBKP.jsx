...
export default function pdfWhatsapp() {
    ...
    const [generateBtn, setGenerateBtn] = useState(pdfTotal);
    // generate pdf start
    ...
    const handlePdfGenerate = async (id) => {
        console.log("clickbtnid", id);
        document.getElementById("overlay").style.display = "block";
        setGenerateBtn(id)
        // document.getElementById("overlay").style.display = "block";
        const uservalue = pdfList?.joindata[id] 
            for (let i = 0; i < uservalue.length; i++) {

            const user = uservalue[i];
            ...
            if (user) {
                ...
                //   if(pg == null){
                //     setTimeout(() => {
                // const  pagesnew = document.querySelector(`#my-table-${i}`);
                //   setpages(pagesnew)

                //   console.log("pagesnew", pages)
                
                //     }, 1000);
                //   }

                  console.log("my-table", pages)
                            ...
                            if (val == generateBtn) {
                                ...
                                return (
                                    ...
                                    pdfList?.joindata[val]?.map((item, index) => {
                                        console.log("`my-table-${index}`", `my-table-${index}`)
                                        return (
