...
export default function joinEvent() {
  ...
  const todayData = moment(today_Data).format("DD:MM:YYYY");
  // event 15/12/23 pdf data
  const [fifteenpdfDataShow, setfifteenPdfDataShow] = useState(0);
  const handlefifteenPdfGenerate = async (id) => {
    setfifteenPdfDataShow(id)
    ...
    for (let i = 0; i < events?.joindata?.length; i++) {
      ...
      if (user) {
        ...
        const pages = document.querySelector(`#eventmy-table-${i}`);
        const canvas = await html2canvas(pages);
        const imageData = canvas.toDataURL('image/png');
            ...
            <div className="card-body">
              <div className="d-flex align-items-center justify-content-between my-3">
                ...
                <div className="col-md-4 text-end">
                  ...
                  <button
                    onClick={(e) => handlefifteenPdfGenerate(1)}
                    className={todayData ? "btn btn-gradient-primary me-4 disabled" : "btn btn-gradient-primary me-4"}
                  >
                    Event 15/12/23 Generate PDF
                  </button>

                  <button
                    onClick={(e) => handlePdfGenerate(1)}
                    className="btn btn-gradient-primary"
                  >
                    Generate PDF
                  </button>
