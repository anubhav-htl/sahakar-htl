...
export default function SinglePrintJoinEvent() {
  const router = useRouter();

  const event_id = router?.query?.event_id;
  const joinEventID = router?.query?.joinEventID;

  const [lang, setLang] = useState("HI");

  const [printList, setPrintList] = useState(null); // State to hold the fetched data
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${API_URL}user-event-join-list/${event_id}`,
          {
            headers: {
  ...
  return (
    <div className="d-flex align-items-center justify-content-center flex-column">
      <Button id="content-None" onClick={() => handlePrint()}>
        Print
      </Button>
      <div id="pdf-print">
        {printList?.joindata.length > 0 ? (
          printList.joindata?.map((item, index) => {
            if (item.id == joinEventID) {
              return (
                <div
                  // id={`my-table-Pdff`}
                  key={index}
