...
export default function pdfWhatsapp() {
    ...
    const handlePdfGenerate = async (id) => {
        ...
        for (let i = 0; i < uservalue?.length; i++) {
            ...
            console.log("user",user?.whatsapp_status);
            if (user) {
                                                                ...
                                                                {/* <button
                                                                    className={
                                                                        ...
                                                                        pdfList?.joindata[item]?.map((pdfval) =>
                                                                            ...
                                                                            pdfval.whatsapp_status == false ? "btn btn-gradient-primary" : "btn btn-gradient-primary "
                                                                        )

                                                                    }
                                                                    data-bs-toggle="modal" data-bs-target={`#${item}`}
                                                                    onClick={() => handlePdfGenerate(item)} style={{ fontSize: "12px", width: "100%" }}>Send PDF/Whatsapp Message </button> */}
                                                                <button
                                                                    className={

                                                                        pdfList?.joindata[item]?.map((pdfval) =>

                                                                            pdfval.buttonstatus == false ? "btn btn-gradient-primary" : "btn btn-gradient-primary disabled"
                                                                        )
