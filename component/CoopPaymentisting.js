...
const CoopPaymentList = () => {
    ...
    const fetchPaymentDetails = async () => {
      try {
        ...
        const response = await axios.get(
          `${API_URL}coopSociety-payment-list`,
          {
  ...
  const handlePdfDownload = (data) => {
    const pdfContentData = {
      ...
      memberShipCategory: data?.payment_option?.plan_name
        ? data?.payment_option?.plan_name
        : "Not Available",
      amount: data?.amount ? `Rs. ${data?.amount} /-` : "Not Available",
      payment_status:
        data?.payment_status === "Success" ? "Success" : "pending",
        ...
        <tbody>
          {paymentDetails.map((detail, idx) => (
            <tr key={detail?.payment_id}>
              <td>
                {currentPage === 1 ? idx + 1 : idx + 1 + (currentPage - 1) * 10}
              </td>
              <td>
                {detail?.payment_id ? detail?.payment_id : "Not available"}
              </td>
              <td>{detail?.amount ? detail?.amount : "Not available"}</td>
              <td>
                {detail?.payment_option?.plan_name
                  ? detail?.payment_option?.plan_name
                  : "Not available"}
              </td>
              {/* <td>{detail?.payment_plan_id}</td> */}
              <td>
                {detail?.payment_status
                  ? detail?.payment_status
                  : "Not available"}
              </td>
              <td>{detail?.typeSociety}</td>
              <td>{detail?.registration_number}</td>
              {/* <td>{detail?.sys_reg_number}</td> */}
              <td>{detail?.name}</td>
              <td>{detail?.status ? "Active" : "Inactive"}</td>
              {/* <td>{detail?.payment_option?.society_fees}</td> */}
              <td>
...
export default CoopPaymentList;
